...
async def test_add_password_invalid_timestamps(device: SwitchbotKeypad) -> None:
    ...
    with pytest.raises(
        ValueError, match=r"end_time .* must be greater than or equal to start_time"
    ):
        await device.add_password("123456", start_time=1800000000, end_time=1700000000)

    with pytest.raises(
        ValueError,
        match=r"start_time and end_time are required when passcode_type is 1",
    ):
        await device.add_password("123456", passcode_type=1)

    with pytest.raises(
        ValueError,
        match=r"start_time and end_time are required when passcode_type is 1",
    ):
        await device.add_password("123456", passcode_type=1, start_time=1700000000)

    with pytest.raises(
        ValueError,
        match=r"start_time and end_time are required when passcode_type is 1",
    ):
        await device.add_password("123456", passcode_type=1, end_time=1800000000)
    ...
    with pytest.raises(
        ...
        await device.add_password(
            "123456", passcode_type=1, start_time=0, end_time=1800000000
        )


@pytest.mark.asyncio
async def test_add_password_empty_token(device: SwitchbotKeypad) -> None:
    """Test adding a password with empty token raises ValueError."""
...
@pytest.mark.asyncio
async def test_add_password_time_window_ble_exception_rollback(
    device: SwitchbotKeypad,
) -> None:
    """Test that BLE exception when setting time window triggers rollback delete."""
    device._send_command.side_effect = [
        b"\x01\x10\x05",  # add response, returns index 5
        RuntimeError("BLE connection dropped"),  # set time window raises BLE error
        b"\x01",  # delete passcode response during rollback
    ]

    with pytest.raises(RuntimeError, match=r"BLE connection dropped"):
        await device.add_password("123456")
...
@pytest.mark.asyncio
async def test_add_password_time_window_ble_exception_rollback_failure(
    device: SwitchbotKeypad,
) -> None:
    """Test that BLE exception on time window write with failed rollback raises escalated error."""
    device._send_command.side_effect = [
        b"\x01\x10\x05",  # add response, returns index 5
        RuntimeError("BLE connection dropped"),  # set time window raises BLE error
        b"\x00",  # delete passcode response fails during rollback
...
@pytest.mark.asyncio
async def test_add_password_time_window_rollback_exception(
    device: SwitchbotKeypad,
) -> None:
    """Test rollback when delete_password raises an exception."""
    device._send_command.side_effect = [
        b"\x01\x10\x05",  # add response, returns index 5
        RuntimeError("BLE connection dropped"),  # set time window raises BLE error
        RuntimeError("BLE delete failed"),  # delete passcode raises during rollback
    ]
    ...
    assert str(exc_info.value.__cause__) == "BLE delete failed"
    device._send_command.assert_any_call("570F52020505")
...
async def test_modify_password_invalid_params(device: SwitchbotKeypad) -> None:
    ...
    with pytest.raises(ValueError, match="Invalid index"):
        await device.modify_password(-1, "654321")

    with pytest.raises(ValueError, match="Invalid index"):
        await device.modify_password(256, "654321")
    ...
    with pytest.raises(
        ...
        await device.modify_password(
            3, "654321", start_time=1800000000, end_time=1700000000
        )
    ...
    with pytest.raises(
        ...
        await device.modify_password(3, "654321", passcode_type=1)
...
async def test_modify_password_failure(device: SwitchbotKeypad) -> None:
    """Test modifying a password raises SwitchbotOperationError on BLE write failure without deleting passcode."""
    device._send_command.side_effect = [
    ...
    # Verify that delete_password was NOT called
    assert device._send_command.call_count == 1


@pytest.mark.asyncio
async def test_modify_password_time_window_failure(device: SwitchbotKeypad) -> None:
    """Test modifying a password raises SwitchbotOperationError on time window failure without deleting passcode."""
    device._send_command.side_effect = [
...
@pytest.mark.asyncio
async def test_delete_password_invalid_index(device: SwitchbotKeypad) -> None:
    """Test deleting a password with invalid index raises ValueError."""
    with pytest.raises(ValueError, match="Invalid index"):
        await device.delete_password(-1)

    with pytest.raises(ValueError, match="Invalid index"):
        await device.delete_password(256)
...
@pytest.mark.asyncio
async def test_delete_password_failure(device: SwitchbotKeypad) -> None:
    """Test deleting a password returns False on BLE failure."""
    device._send_command.return_value = b"\x00"
...
@pytest.mark.asyncio
async def test_get_password_count_error_status(device: SwitchbotKeypad) -> None:
    """Test getting password counts returns None on error status byte."""
...
@pytest.mark.asyncio
async def test_sync_time_invalid_timestamp(device: SwitchbotKeypad) -> None:
    """Test synchronizing device time with invalid timestamp raises ValueError."""
    with pytest.raises(ValueError, match="Invalid timestamp"):
        await device.sync_time(timestamp=-1)

    with pytest.raises(ValueError, match="Invalid timestamp"):
        await device.sync_time(timestamp=0x10000000000000000)
...
@pytest.mark.asyncio
async def test_sync_time_failure(device: SwitchbotKeypad) -> None:
    """Test synchronizing device time returns False on BLE failure."""
    device._send_command.return_value = b"\x00"
...
@pytest.mark.asyncio
async def test_verify_encryption_key() -> None:
    """Test verify_encryption_key succeeds when basic info returns data."""
    ble_device = generate_ble_device("aa:bb:cc:dd:ee:ff", "any")
    key_id = "ff"
